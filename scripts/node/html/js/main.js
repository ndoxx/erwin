...
KLogger.messageStyles = [
    'type-raw', 'type-normal', 'type-item', 'type-event', 'type-notify', 'type-warning', 'type-error', 'type-fatal',
    'type-bang', 'type-good', 'type-bad'
];
...
updateChannelFilter() {
    $("#channelSelector option").each(function() {
        ...
        {
            $(`.filter-${channel}`).each(function() { $(this).removeClass("chan-hidden"); });
        }
        else
        {
            $(`.filter-${channel}`).each(function() { $(this).addClass("chan-hidden"); });
        }
    });
}
...
function
selectAllCHannels() {
    $("#channelSelector option").each(function() { $(this).prop('selected', true); });
    $(`.message`).each(function() { $(this).removeClass("chan-hidden"); });
}

function
updateVerbosityFilter() {
    var verbosity = $("#verbositySlider").val();
    for(var severity = 0; severity <= 3; ++severity)
    {
        if(verbosity >= 3 - severity)
        {
            $(`.filter-sev-${severity}`).each(function() { $(this).removeClass("sev-hidden"); });
        }
        else
        {
            $(`.filter-sev-${severity}`).each(function() { $(this).addClass("sev-hidden"); });
        }
    }
}
...
function
clearMessages() { $("#message-container").empty(); }

function createChannelSubscriptionsHiddenField(subscriptions) {
    $("#message-container")
        .append(`<input type="hidden" id="hidSubscriptions" name="hidSubscriptions" value="${subscriptions}">`);
}

function
loadChannelSubscriptions() { KLogger.channels = $("#message-container > #hidSubscriptions").val().split(','); }
...
function handlePacket(item) {
    try
    {
        ...
        case "msg":
            ...
            $("#message-container")
                .append(`<div class="message ${styeClass} filter-${e.channel} filter-sev-${
                    e.severity}"><div class="messagehead"><div class="channel" style="background: ${chanColor}">${
                    e.channel}</div><div class="timestamp">${e.timestamp}</div></div>${message}${
                    locationCaption}</div>`);
            break;
        ...
        case "connect":
            ...
            $("#message-container")
                .append(`<div class="message message-status">peer: ${e.peer_ip}:${e.peer_port}</div>`);
            break;
...
$(function() {
    ...
    KLogger.websocket.onmessage =
        ...
        // ---- FILE CONTROLS ----
        $("#controls-container")
            .append(`<div class="divider"><span></span><span>File</span><span></span></div>`);

    // Export button
    $("#controls-container")
        .append(
            `<input type=button value="Export" onclick="downloadInnerHtml('erwin.log','message-container','text/html')" style="width: 100%"></input>`)
    // Import button
    $("#controls-container").append(`<input id="fileDialog" type="file" style="display: none" />`);
    $("#controls-container")
        .append(`<input id="btnShowFiledialog" type=button value="Import" style="width: 100%"></input>`);

    $("#btnShowFiledialog").on("click", function() { $("#fileDialog").trigger("click"); });
    ...
    $("#controls-container")
        .append(`<input type=button value="Select all" onclick="selectAllCHannels()" style="width: 100%"></input>`);
    $("#controls-container")
        .append(`<select multiple size="${
            KLogger.channels
                .length}" id="channelSelector" onchange="updateChannelFilter()" style="width: 100%;"></select>`);
    updateChannelControls();

    // Severity filtering
    $("#controls-container").append(`<div id="rangeWithLabel" style="display: flex;">`);
    $("#rangeWithLabel")
        .append(`<label for="verbositySlider" style="flex-grow: 1; margin-right: 5px;">Verbosity</label>`);
    $("#rangeWithLabel")
        .append(
            `<input id="verbositySlider" type=range min="0" max="3" value="3" class="slider" oninput="updateVerbosityFilter()" style="width: 100%; flex-grow: 3;">`);
});