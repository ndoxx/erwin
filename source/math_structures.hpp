...
namespace std
{
    ...
    inline void swap(::erwin::math::vec<N,T>& left, ::erwin::math::vec<N,T>& right)
    { left.swap(right); }

    // math::vecN Hasher
    // enables the use of unordered containers with vecs
    template <unsigned N, typename T>
    struct hash<::erwin::math::vec<N,T>>
    {
        typedef ::erwin::math::vec<N,T> argument_type;
        ...
        result_type operator()(argument_type const& arg) const
        {
            ...
            for(unsigned ii = 0; ii < N; ++ii) {
                seed ^= ::erwin::detail::Hash(arg[ii]) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
            }
