...
void LayerQTest2D::on_imgui_render()
{
    ImGui::Begin("BatchRenderer2D");
    	if(ImGui::Checkbox("Profile", &enable_profiling_))
    		MasterRenderer::instance().set_profiling_enabled(enable_profiling_);
...
void LayerQTest2D::on_attach()
{
	renderer_2D_ = make_scope<Renderer2D>(batch_size_);
...
void LayerQTest2D::on_update(GameClock& clock)
{
	// Update
...
bool LayerQTest2D::on_event(const MouseButtonEvent& event)
{
	DLOGN("event") << get_name() << " -> Handled event: " << event << std::endl;
	return false;
}

bool LayerQTest2D::on_event(const WindowResizeEvent& event)
{
	camera_ctl_.on_window_resize_event(event);
	return false;
}

bool LayerQTest2D::on_event(const MouseScrollEvent& event)
{
