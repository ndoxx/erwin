...
static const std::map<char, ImVec4> s_color_table = {
    {'N', ImVec4(0.59f, 0.51f, 1.f, 1.f)}, {'W', ImVec4(1.f, 0.69f, 0.f, 1.f)}, {'E', ImVec4(1.f, 0.35f, 0.35f, 1.f)},
    {'F', ImVec4(1.f, 0.f, 0.f, 1.f)},     {'G', ImVec4(0.f, 1.f, 0.f, 1.f)},   {'B', ImVec4(1.f, 0.f, 0.f, 1.f)}};
...
ConsoleWidget::ConsoleWidget(erwin::EventBus& event_bus) : Widget("Console", true, event_bus)
{
...
static std::string message_type_to_style_tag(kb::klog::MsgType type)
{
    switch(type)
    {
    case kb::klog::MsgType::RAW:
        return "";
    case kb::klog::MsgType::NORMAL:
        return "";
    case kb::klog::MsgType::ITEM:
        return "";
    case kb::klog::MsgType::EVENT:
        return "";
    case kb::klog::MsgType::NOTIFY:
        return "[!N]";
    case kb::klog::MsgType::WARNING:
        return "[!W]";
    case kb::klog::MsgType::ERROR:
        return "[!E]";
    case kb::klog::MsgType::FATAL:
        return "[!F]";
    case kb::klog::MsgType::BANG:
        return "";
    case kb::klog::MsgType::GOOD:
        return "[!G]";
    case kb::klog::MsgType::BAD:
        return "[!B]";
    default:
        return "";
    }
...
ConsoleWidgetSink::ConsoleWidgetSink(ConsoleWidget* p_console) : console_(p_console) {}
...
void ConsoleWidgetSink::send(const kb::klog::LogStatement& stmt, const kb::klog::LogChannel&)
{
    ...
    std::stringstream ss;
    ss << message_type_to_style_tag(stmt.msg_type) << "[" << std::setprecision(6) << std::fixed << ts << "] "
       << strip_ansi(stmt.message);
    console_->push(ss.str());
}

void ConsoleWidgetSink::send_raw(const std::string& message) { console_->push(message); }
