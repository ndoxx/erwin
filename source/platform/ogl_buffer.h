...
public:
    ...
    void init(void* data, uint32_t size, UsagePattern mode);
    void bind() const;
    void unbind() const;
...
public:
    ...
    void set_index_buffer(WRef<OGLIndexBuffer> p_ib);
    // Add A vertex buffer, should use this for non-interleaved data
    void add_vertex_buffer(WRef<OGLVertexBuffer> p_vb);
    // Set THE vertex buffer, should use this for interleaved data
    inline void set_vertex_buffer(WRef<OGLVertexBuffer> p_vb)
    {
    ...
    inline const std::vector<WRef<OGLVertexBuffer>>& get_vertex_buffers() const { return vertex_buffers_; }
...
protected:
    W_ID unique_id_;
    std::vector<WRef<OGLVertexBuffer>> vertex_buffers_;
    WRef<OGLIndexBuffer> index_buffer_ = nullptr;
    uint32_t rd_handle_;
