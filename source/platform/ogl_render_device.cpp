...
void OGLRenderDevice::add_framebuffer_texture_vector(FramebufferHandle handle, const FramebufferTextureVector& ftv)
{
    s_rd_storage.framebuffer_textures_.insert(std::make_pair(handle.index, ftv));
}
...
std::pair<uint64_t, std::future<PixelData>> OGLRenderDevice::future_texture_data()
{
    return s_rd_storage.texture_data_promises_.future_operation();
}

FramebufferHandle OGLRenderDevice::default_render_target()
{
    return s_rd_storage.default_framebuffer_;
}

TextureHandle OGLRenderDevice::get_framebuffer_texture(FramebufferHandle handle, uint32_t index)
{
...
CubemapHandle OGLRenderDevice::get_framebuffer_cubemap(FramebufferHandle handle)
{
    W_ASSERT(handle.is_valid(), "Invalid FramebufferHandle.");
    return s_rd_storage.framebuffer_textures_[handle.index].cubemap;
}

hash_t OGLRenderDevice::get_framebuffer_texture_name(FramebufferHandle handle, uint32_t index)
{
    W_ASSERT(handle.is_valid(), "Invalid FramebufferHandle.");
    return s_rd_storage.framebuffer_textures_[handle.index].debug_names[index];
}

uint32_t OGLRenderDevice::get_framebuffer_texture_count(FramebufferHandle handle)
{
    W_ASSERT(handle.is_valid(), "Invalid FramebufferHandle.");
    return uint32_t(s_rd_storage.framebuffer_textures_[handle.index].handles.size());
}

void* OGLRenderDevice::get_native_texture_handle(TextureHandle handle)
{
...
VertexBufferLayoutHandle OGLRenderDevice::create_vertex_buffer_layout(const std::vector<BufferLayoutElement>& elements)
{
...
const BufferLayout& OGLRenderDevice::get_vertex_buffer_layout(VertexBufferLayoutHandle handle)
{
								   ...
								   std::size_t offset)
{
	...
	glDrawElements(OGLPrimitive.at(vao->get_index_buffer().get_primitive()), 
...
{
    OGLVertexArray* vao = reinterpret_cast<OGLVertexArray*>(vertexArray);
    glDrawArrays(OGLPrimitive.at(prim), 
                 offset, 
...
{
    OGLVertexArray* vao = reinterpret_cast<OGLVertexArray*>(vertexArray);
    glDrawElementsInstanced(OGLPrimitive.at(vao->get_index_buffer().get_primitive()),
...
void OGLRenderDevice::dispatch_command(uint16_t type, memory::LinearBuffer<>& buf)
{
    (*render_backend_dispatch[type])(buf);
}

void OGLRenderDevice::dispatch_draw(uint16_t type, memory::LinearBuffer<>& buf)
{
    (*draw_backend_dispatch[type])(buf);
}
