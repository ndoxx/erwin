...
/*
	...
	kernel with a half-size of N+1 with N even, will correspond to a convolution matrix of size 
...
struct SeparableGaussianKernel
{
	...
	SeparableGaussianKernel(uint32_t half_size, float sigma=1.f);

	void init(uint32_t half_size, float sigma);
