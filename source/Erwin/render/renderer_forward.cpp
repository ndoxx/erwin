...
void ForwardRenderer::draw_mesh(VertexArrayHandle VAO, const ComponentTransform3D& transform, const Material& material)
{
	...
	dc.set_UBO(s_storage.instance_ubo, (void*)&instance_data, sizeof(InstanceData), DataOwnership::Copy, 0);
	if(material.ubo.index != k_invalid_handle && material.data)
		dc.set_UBO(material.ubo, material.data, material.data_size, DataOwnership::Copy, 1);
