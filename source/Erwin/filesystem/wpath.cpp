...
WPath::WPath(const std::string& path):
universal_(path),
resource_id_(H_(universal_.c_str()))
{
	std::smatch match;
...
fs::path WPath::relative() const
{
	switch(protocol_)
	{
		case "usr"_h: return fs::relative(absolute_, s_user_dir_);
		case "res"_h: return fs::relative(absolute_, s_resource_dir_);
		case "cfg"_h: return fs::relative(absolute_, s_config_dir_);
		case "appres"_h: return fs::relative(absolute_, s_app_resource_dir_);
		case "sysres"_h: return fs::relative(absolute_, s_sys_resource_dir_);
		case "syscfg"_h: return fs::relative(absolute_, s_sys_config_dir_);
		default: return fs::relative(absolute_, s_root_dir_);
	}
}
