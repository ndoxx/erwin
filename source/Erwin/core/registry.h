...
public:
    ...
    template <typename T> const T& get(hash_t hname, const T& def) const;
    const std::string& get(hash_t hname) const;
    bool is(hash_t name) const;
    template <typename T> bool set(hash_t hname, const T& value);
