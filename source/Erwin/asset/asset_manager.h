...
public:
     ...
     */
    template <typename ResT> static const ResT& load(size_t reg, const std::string& file_path);
     ...
     * @return     const reference to the newly created (or cached) resource.
     */
    template <typename ResT, typename OptT>
    static const ResT& load(size_t reg, const std::string& file_path, const OptT& options);
     ...
     */
    static hash_t load_resource_async(size_t reg, AssetMetaData::AssetType type, const std::string& file_path);
     ...
     *             to use the resource when it is ready.
     */
    template <typename ResT> static hash_t load_async(size_t reg, const std::string& file_path);
