...
void init()
{
    ...
    auto theme_dir = WFS_.regular_path("sysres://themes");
    for(auto& entry : fs::directory_iterator(theme_dir))
    {
        ...
        xml::XMLFile theme_f(std::string(entry.path()));
        if(!theme_f.read())
            continue;
    ...
    std::sort(s_themes.begin() + 1, s_themes.end(),
...
const std::vector<ThemeEntry>& get_list() { return s_themes; }

bool load(const ThemeEntry& entry)
{
    std::string xml_path(entry.path);
    ...
    // Parse style colors
    for(rapidxml::xml_node<>* col_node = root->first_node("color"); col_node;
    ...
    for(rapidxml::xml_node<>* prop_node = root->first_node("prop"); prop_node;
        prop_node = prop_node->next_sibling("prop"))
    {
...
void reset() { ImGui::GetStyle() = s_default_style; }
