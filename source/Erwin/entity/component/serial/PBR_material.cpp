...
template <> void serialize_xml<ComponentPBRMaterial>(const ComponentPBRMaterial& cmp, xml::XMLFile& file, rapidxml::xml_node<>* cmp_node)
{
    if(cmp.override)
    {
        ...
        file.add_node(cmp_node, "tint", kb::to_string(cmp.material_data.tint).c_str());
        ...
        file.add_node(cmp_node, "u_albedo", kb::to_string(cmp.material_data.uniform_albedo).c_str());
    ...
    file.add_attribute(cmp_node, "id", kb::to_string(cmp.material.resource_id).c_str());
