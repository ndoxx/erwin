...
vec2 compress_normal_z_reconstruct(vec3 norm)
{
    return normalize(norm + vec3(0.f,0.f,1.f)).xy;
}
// Decompress a 2D normal back to 3 components
vec3 decompress_normal_z_reconstruct(vec2 norm)
{
...
vec2 compress_normal_spheremap_transform(vec3 norm)
{
    vec2 enc = normalize(norm.xy) * (sqrt(-norm.z * 0.5f + 0.5f));
    return enc * 0.5f + 0.5f;
}
vec3 decompress_normal_spheremap_transform(vec2 norm)
{
...
vec2 compress_normal_lambert_azimuthal(vec3 norm)
{
    float f = 1.f / sqrt(8.f * norm.z + 8.f);
    return norm.xy * f + 0.5f;
}
vec3 decompress_normal_lambert_azimuthal(vec2 norm)
{
